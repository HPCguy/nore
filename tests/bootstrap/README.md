...
- codegen bootstrap tests now cover the committed Milestone 8 slices: stable C typedef emission for arrays, slices, values, structs, tables, table rows, and enums, stable user-function prototype emission with module prefixes, ref lowering, and native-declaration skipping, stable top-level global definition emission for the first `val` / `mut` slice, plus the assembled whole-file C file with the minimal prelude, the broader early statement/expression subset, focused `match` lowering coverage for statement/tail/local-init paths, one real compiler support-module codegen smoke test, one real compiler frontend-module match smoke test, and one real parser-module globals smoke test
