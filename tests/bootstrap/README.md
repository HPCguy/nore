...
- codegen bootstrap tests now cover the committed Milestone 8 slices: stable C typedef emission for arrays, slices, values, structs, tables, table rows, and enums, stable user-function prototype emission with module prefixes plus plain `ni_*` native runtime-hook prototypes, ref lowering, stable top-level global definition emission for the first `val` / `mut` slice, plus the assembled whole-file C file with the minimal prelude, native/user prototype sections, the broader early statement/expression subset including typed array literals and slice-view ref-argument temporaries, focused `match` lowering coverage for statement/tail/local-init paths, one real compiler support-module codegen smoke test, one real compiler frontend-module match smoke test, one real parser-module globals smoke test, and one real std-module codegen smoke test
