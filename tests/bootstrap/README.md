...
- codegen bootstrap tests now cover the first committed Milestone 8 slices: stable C typedef emission for arrays, slices, values, structs, tables, table rows, and enums, stable user-function prototype emission with module prefixes, ref lowering, and native-declaration skipping, plus the first assembled whole-file C file with the minimal prelude and the first simple function-body subset
