...
- selfhost bootstrap tests now cover the first committed stage-0 -> bootstrap-compiler -> generated-C -> Clang -> native-binary smoke path for `tests/success/print_hello.nore` and `tests/success/args.nore`, a full stage-0 -> `nore1` -> `nore2` -> `nore3` self-compile chain with identical stage-2/stage-3 generated C, and the thin shell driver wrapper that builds ordinary programs through the self-hosted compiler without manual C/Clang steps
...
Run these tests with `make test-bootstrap` or `tests/run_bootstrap_tests.sh`.
